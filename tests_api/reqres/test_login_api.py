...
def test_login(module_logger, correo, contrasena, estado_esperado, token_esperado):
    """
    ...
    module_logger.info(f"Iniciando test de login. Parámetros recibidos -> correo='{correo}', contraseña='{contrasena}', estado_esperado={estado_esperado}, token_esperado={token_esperado}")

    datos = {}
    datos["email"] = correo or None
    datos["password"] = contrasena or None

    module_logger.info(f"Payload enviado: {datos}")
    ...
    if token_esperado == "True":
        ...
        module_logger.info("Token recibido correctamente.")
    else:
        assert "error" in contenido, (
            f"Se esperaba error para correo='{correo}', contraseña='{contrasena}', "
            f"pero no se recibió"
        )
        module_logger.info(f"Error recibido correctamente: {error_recibido}")
