...
@pytest.mark.api
def test_listar_usuarios(module_logger):
    """
    ...
    module_logger.info(f"Petición enviada. Status recibido: {respuesta.status_code}")

    assert respuesta.status_code == 200, f"Se esperaba status 200, pero se obtuvo {respuesta.status_code}"

    contenido = respuesta.json()
    module_logger.info("Respuesta JSON recibida correctamente")

    usuarios = contenido.get("data", [])
    module_logger.info(f"Cantidad de usuarios encontrados: {len(usuarios)}")

    assert usuarios, "No se encontraron usuarios en la respuesta"

    for index, usuario in enumerate(usuarios):
        module_logger.info(f"Validando usuario índice {index}: {usuario}")
        ...
        module_logger.info(f"Avatar usuario índice {index}: {avatar}")
