...
def test_crear_usuario(module_logger, nombre, trabajo):
    """
    ...
    module_logger.info(f"Iniciando test de creación de usuario: Parámetros -> nombre='{nombre}', trabajo='{trabajo}'")
    ...
    module_logger.info(f"Payload enviado: {payload}")

    respuesta = post("users", payload)
    module_logger.info(f"Status recibido: {respuesta.status_code}")

    assert respuesta.status_code == 201, f"Se esperaba status 201, pero se obtuvo {respuesta.status_code}"
 
    contenido = respuesta.json()
    module_logger.info(f"Contenido recibido: {contenido}")

    assert contenido.get("name") == nombre, f"Se esperaba name='{nombre}', pero se obtuvo '{contenido.get('name')}'"

    assert contenido.get("job") == trabajo, f"Se esperaba job='{trabajo}', pero se obtuvo '{contenido.get('job')}'"

    created_at = contenido.get("createdAt")
    module_logger.info(f"createdAt recibido: {created_at}")

    assert created_at, "No se encontró createdAt en la respuesta"
    assert str(datetime.now().year) in created_at, f"El año actual no está en createdAt: {created_at}"
